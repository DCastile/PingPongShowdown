	...
	matchStorage.newMatch(session, function (currentMatch) {
		...
		if (currentMatch.data.MatchData.ExperiencedUserMode == false) {
			if (currentMatch.data.MatchType == 'doubles') {
				speechText = 'Welcome doublesplayers. Say, load preferences to apply your favorite settings to this match.<break time=\"0.4s\" /> ';
			} else {
				speechText = 'Welcome singlesplayers. Say, load preferences to apply your favorite settings to this match.<break time=\"0.4s\" /> ';
			};
...
function fulfillSavePreferences(session, response) {
	...
	matchStorage.loadMatch(session, function (currentMatch) { // need to get match data because we will add player data to it and re-save
		...
		playerStorage.loadPlayer(session, function (loadedPlayer) {
			...
			loadedPlayer.data.Preferences.AnnounceServe = currentMatch.data.MatchData.AnnounceServe;
			loadedPlayer.data.Preferences.AnnounceScore = currentMatch.data.MatchData.AnnounceScore;
...
function setTeamSummaryStats(rawStats, teamToReport, callback) {
	...
	if (teamToReport == 'red') {
		...
		stats.PercentDeucePointConversions = ((rawStats.RedDeucePointsWon / rawStats.GamesWonWithDeuce) * 100).toFixed(0);		
		...
		stats.PercentDeucePointConversions = ((rawStats.BlueDeucePointsWon / rawStats.GamesWonWithDeuce) * 100).toFixed(0);		
		...
		stats.PercentGamesWonOnServe = ((rawStats.SinglesOrDoublesGamesWonOnServe / rawStats.SinglesOrDoublesGamesServed) * 100).toFixed(0);		
		...
		stats.PercentBreakPointConversions = ((rawStats.SinglesOrDoublesBreakPointConversions / rawStats.BreakPointsAgainstOpponent) * 100).toFixed(0);	
		stats.PercentBreakPointsSaved = ((rawStats.SinglesOrDoublesBreakPointsSaved / rawStats.BreakPointsAgainstSelf) * 100).toFixed(0);		
		stats.PercentGamePointConversions = ((rawStats.SinglesOrDoublesGamePointsWon / rawStats.GamePoints) * 100).toFixed(0);	
		
		stats.PointsServed = rawStats.SinglesOrDoublesPointsServed;
		stats.GamesServed = rawStats.SinglesOrDoublesGamesServed;
		stats.BreakPointConversions = rawStats.SinglesOrDoublesBreakPointConversions;
		stats.BreakPointsAgainstOpponent = rawStats.BreakPointsAgainstOpponent;
		stats.BreakPointsSaved = rawStats.SinglesOrDoublesBreakPointsSaved;
		stats.BreakPointAgainstSelf = rawStats.BreakPointsAgainstSelf;
		stats.GamePointsWon = rawStats.SinglesOrDoublesGamePointsWon;
		stats.DeucePointsWon = rawStats.SinglesOrDoublesDeucePointsWon;
	...
	setTeamSummaryStats(rawStats, session.attributes.summaryForTeam, function (currentMatchStats) {	// get red or blue team current match stats 	
		...
		setTeamSummaryStats(rawHistStats, 'singlesOrDoublesHistory', function (histStats) { // get singles player historical stats
			...
			if (histStats.NumberOfMatches <= 1) {
				...
				if (currentMatchStats.GamesServed != 0 && ( !(isNaN(histStats.PercentGamesWonOnServe)) )) {
				...
				if (currentMatchStats.BreakPointsAgainstOpponent != 0 && ( !(isNaN(histStats.PercentBreakPointConversions)) )) {
					...
					if (diffPercentBreakPointConversions >= 0 ) {
						higherOrLower = 'higher';
					} else {
						higherOrLower = 'lower';
				...
				if (currentMatchStats.BreakPointAgainstSelf != 0 && ( !(isNaN(histStats.PercentBreakPointsSaved)) )) {
				...
				if (currentMatchStats.GamePoints != 0 && ( !(isNaN(histStats.PercentGamePointConversions)) ) ) {
					...
					histSpeechText += '<break time=\"0.3s\" />. The number of game points you won was ';
...
function createTextSummaryForTeam (session, currentMatch, teamToReport, callback) {
	console.log('entering createTextSummaryForTeam function');
	
	// get current match stats
	var rawStats = currentMatch.data.MatchData;	

	setTeamSummaryStats(rawStats, teamToReport, function (currentMatchStats) {	// get current match stats for red or blue team								
		console.log('currentMatchStats = ' + JSON.stringify(currentMatchStats) );
			
		// get historical match stats for red or blue team	
		// first set phoneKey(s) for whether red team summary or blue team summary, and singles or doubles
		if (teamToReport == 'red') {
		...
		// now go get the historical stats 
		if (currentMatch.data.MatchType == 'singles') {			
			// pull stats from all matches in which this player has played singles
			...
			singlesPlayerHistory.getSinglesPlayerHistory(session, function (rawHistStats) {						
				buildContent(rawHistStats, function (teamStats) {								
					callback(teamStats);
				});																					
			});			
		} else {
		...
		function buildContent(rawHistStats, callback) {
			console.log('entering buildContent function');
			
			setTeamSummaryStats(rawHistStats, 'singlesOrDoublesHistory', function (histStats) {	// get historical stats for red team or blue team					
				console.log('histStats = ' + JSON.stringify(histStats) );
				var teamStats = '';	
				var higherOrLower;
				...
				// percent of games won
				if ( !(isNaN(currentMatchStats.PercentGamesWon)) ) {
					teamStats += currentMatchStats.PercentGamesWon + '% of games' + "\n";
				};	
				
				// percent of points won
				teamStats += currentMatchStats.PercentPointsWon + '% of pts' + "\n";

				// percent of points won serving
				if (currentMatchStats.PointsServed != 0) {
					teamStats += currentMatchStats.PercentPointsWonOnServe + '% of pts on serve' + "\n";		
				};
				...
				if (currentMatchStats.GamesServed != 0) {
					...
					if ( !(isNaN(histStats.PercentGamesWonOnServe)) ) { // if historical stats exist
						var diffPercentGamesWonOnServe = currentMatchStats.PercentGamesWonOnServe - histStats.PercentGamesWonOnServe;
						if (diffPercentGamesWonOnServe >= 0 ) {
							higherOrLower = ' > ';
						} else {
						...
						teamStats += '(' + diffPercentGamesWonOnServe;
						teamStats += '% ' + higherOrLower + 'Hist avg)' + "\n";				
					}
										
				};	

				// break point conversions
				if (currentMatchStats.BreakPointsAgainstOpponent != 0)  { // if stat exists
					teamStats += "\n" + 'Break point conv: ' + currentMatchStats.PercentBreakPointConversions + '%';
					teamStats += ' (' + currentMatchStats.BreakPointConversions + '/' + currentMatchStats.BreakPointsAgainstOpponent + ')' + "\n";	
				
					if ( !(isNaN(histStats.PercentBreakPointConversions)) ) { // if historical stats exist
						var diffPercentBreakPointConversions = currentMatchStats.PercentBreakPointConversions - histStats.PercentBreakPointConversions;
						if (diffPercentBreakPointConversions >= 0 ) {
							higherOrLower = ' > ';
						} else {
						...
						teamStats += '(' + diffPercentBreakPointConversions;
						teamStats += '% ' + higherOrLower + 'Hist avg)' + "\n";
					};
				};			

				teamStats += 'SPLIT' + '(Con\'t)' + "\n";

				
				// break points saved
				if (currentMatchStats.BreakPointAgainstSelf != 0) { // if stat exists
					teamStats += "\n" + 'Break points saved: ' + currentMatchStats.PercentBreakPointsSaved + '% ';
					teamStats += '(' + currentMatchStats.BreakPointsSaved + '/' + currentMatchStats.BreakPointAgainstSelf + ')' + "\n";	
					
					if ( !(isNaN(histStats.PercentBreakPointsSaved)) ) { // if historical stats exist
						var diffPercentBreakPointsSaved = currentMatchStats.PercentBreakPointsSaved - histStats.PercentBreakPointsSaved;
						if (diffPercentBreakPointsSaved >= 0 ) {
							higherOrLower = ' > ';
						} else {
						...
						teamStats += '(' + diffPercentBreakPointsSaved;
						teamStats += '% ' + higherOrLower + 'Hist avg)' + "\n";
					};				
				};
				...
				if (currentMatch.data.MatchData.PlayGamePoint) { // if the match was played with game point
					...
					// game point conversions
					if (currentMatch.data.MatchData.GamePoints != 0) { // if stat exists
						teamStats += "\n" + 'Game point conv: ' + currentMatchStats.PercentGamePointConversions + '%';
						teamStats += ' (' + currentMatchStats.GamePointsWon + '/' + currentMatch.data.MatchData.GamePoints + ')' + "\n";
						
						if ( !(isNaN(histStats.PercentGamePointConversions)) ) { // if historical stats exist
							var diffPercentGamePointConversions = currentMatchStats.PercentGamePointConversions - histStats.PercentGamePointConversions;
							if (diffPercentGamePointConversions >= 0 ) {
								higherOrLower = ' > ';
							} else {
							...
							teamStats += '(' + diffPercentGamePointConversions;
							teamStats += '% ' + higherOrLower + 'Hist avg)' + "\n";	
						};
					};				
				...
				// deuce points ultimately won
					...
					// deuce point conversions
					if (currentMatch.data.MatchData.GamesWonWithDeuce != 0) { // if stat exists
						teamStats += "\n" + 'Deuce point conv: ' + currentMatchStats.PercentDeucePointConversions + '%';
						teamStats += ' (' + currentMatchStats.DeucePointsWon + '/' + currentMatch.data.MatchData.GamesWonWithDeuce + ')' + "\n";	
					};				
				};

				// point streak
				teamStats += "\n" + 'Max point streak: ' + currentMatchStats.PointStreak + ' pts' + "\n";
				
				if ( !(isNaN(histStats.PointStreak)) ) { // if historical stats exists
					var diffPointStreak = currentMatchStats.PointStreak - histStats.PointStreak;
					if (diffPointStreak >= 0 ) {
						higherOrLower = ' > ';
					} else {
						higherOrLower = ' < ';
						diffPointStreak = Math.abs(diffPointStreak);
					};
					
					teamStats += '(' + diffPointStreak;
					teamStats += ' ' + higherOrLower + 'Hist avg)' + "\n" + "\n";	
				};	
...
function createTextSummaryForMatch (session, currentMatch, callback) {
	console.log('entering createTextSummaryForMatch function');
	
	createTextSummaryForTeam(session, currentMatch, 'red', function (redStats) {
		console.log('redStats = ' + redStats);

		createTextSummaryForTeam(session, currentMatch, 'blue', function (blueStats) {
			console.log('blueStats = ' + blueStats);			
			...
			console.log('currentMatch.data.MatchType = ' + currentMatch.data.MatchType);
			if (currentMatch.data.MatchType == 'singles') {
			...
			var cardTitle = dateString + matchType + 'Match Stats';
			console.log('cardTitle = ' + cardTitle);

			// opening: e.g. 'singles match, 2-1, blue team'
			var cardContent = formatMatchScore(currentMatch, 'text') + "\n";

			// *********************** insert s3 image link here when card type 'standard' is available in alexaSkill.js
			// 
			
			cardContent += 'Play Time: ' + matchTimeText.elapsedTimeString + "\n" + "\n";
			cardContent += 'Sets:' +  "\n"  ;
			cardContent += '1: ' + formatSetScore(1, false, 'text', currentMatch) + "\n";
			cardContent += '2: ' + formatSetScore(2, false, 'text', currentMatch) + "\n"; 
			cardContent += '3: ' + formatSetScore(3, false, 'text', currentMatch) + "\n" + "\n"; 
			cardContent += 'Games played: ' + totalGamesPlayed + "\n";
			cardContent += 'Pts played: ' + totalPointsPlayed + 'SPLIT';
			...
			textToSend[3] = splitTextArray[3];
			textToSend[4] = splitTextArray[4];	
			...
			console.log('textToSend[3] = ' + textSummaryContent.textToSend[3]);	
			console.log('textToSend[4] = ' + textSummaryContent.textToSend[4]);
			
			console.log('textSummaryContent = ' + JSON.stringify(textSummaryContent) );
			
			callback(textSummaryContent);
		});
	});
};
...
var registerIntentHandlers = function (intentHandlers, skillContext) {
    ...
    intentHandlers.TextMatchSummaryIntent = function (intent, session, response) {
		...
		matchStorage.loadMatch(session, function (currentMatch) {
			...
			console.log('currentMatch.data.MatchType in TextMatchSummaryIntent = ' + currentMatch.data.MatchType);
			createTextSummaryForMatch(session, currentMatch, function (matchSummaryContent) {
				console.log('back from callback and into steps toward sending text');
				...
				async.forEachSeries(	textsToSend, // array of items
					...
					function(err){
						...
						if (problems == true) {
							speechText += ' Not all players received it, some may not have opted in.'
						}
						//response.tell(speechText);	
    ...
    intentHandlers.TellTextMessageHelpIntent = function (intent, session, response) {
        console.log('entering TellTextMessageHelpIntent');				
			var speechOutput = {
				speech: '<speak>' + textHelper.textMessageHelp + ' What would you like to do?' + '</speak>',
				type: AlexaSkill.speechOutputType.SSML
			};
			response.ask(speechOutput);			
    };	

    intentHandlers.TempMessageIntent = function (intent, session, response) {
        console.log('entering TellTextMessageHelpIntent');				
			var speechOutput = {
				speech: '<speak>' + textHelper.tempMessageHelp + '</speak>',
				type: AlexaSkill.speechOutputType.SSML
			};
			response.tell(speechOutput);			
    };		
